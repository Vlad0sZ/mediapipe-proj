...
namespace SensorPack.KinectCore.Runtime.Avatar
{
	/// <summary>
	...
	public class AvatarController : MonoBehaviour
	{
		...
		[SerializeField] private Transform hipsBodyTransform;
		...
		[SerializeField] private float distanceScaler;
		//
...
//            jointPosition.y - offsetPos.y + kinectManager.sensorHeight,  //!! this should be better investigated .. 
		...
		{	
			...
			if (userId == 0)
			{
				hipsBodyTransform.localPosition = Vector3.up; //TODO Refactor this
				hipsBodyTransform.localRotation = Quaternion.identity;
				hipsBodyTransform.parent.localRotation = Quaternion.identity;
				// hipsBodyTransform.localPosition = Vector3.up; //TODO Refactor this
				return;
			}
			...
			for(int pass = 0; pass < 2; pass++)  // 2 passes because clavicles are at the end
			{
				for(int i = 0; i < bones.Length; i++)
				{
					if(bones[i] != null)
					{
						...
						// Debug.Log(bones[i].gameObject.name + " rot " + bones[i].rotation);
					}
				}
			}
...
//			return;
		...
		protected void MoveAvatar(Int64 UserID)
		{
			...
			{
				var posTemp = smoothFactor != 0f ? 
					Vector3.Lerp(transform.position, targetPos, smoothFactor * Time.deltaTime) : targetPos;

				var posX = targetPos.x * ( distanceScaler * 1 / targetPos.z );
				if(freezeZ) posTemp.z = zOffset;

				posTemp = new Vector3(posX, posTemp.y, posTemp.z);
				// if(freezeY) posTemp.y = yOffset;
				// if (kinectManager.GetBodyCount() == 0)
				// {
				// 	posTemp.y = yOffset;
				// 	hipsBodyTransform.localPosition = Vector3.up;
				// }
				//
		...
		protected void SetModelArmsInTpose(bool setHandDown = false)
		{
			var offsetHandDirectionVector = setHandDown ? Vector3.down : Vector3.zero;
		
			Vector3 vTposeLeftDir = transform.TransformDirection(Vector3.left + offsetHandDirectionVector);
			Vector3 vTposeRightDir = transform.TransformDirection(Vector3.right + offsetHandDirectionVector);
