...
{
    public sealed class Kinect1Interface : DepthSensorInterface
    {
        ...
        private Dictionary<uint, InteractionHandEventType> lastLeftHandEvent =
            new Dictionary<uint, InteractionHandEventType>();

        private Dictionary<uint, InteractionHandEventType> lastRightHandEvent =
            new Dictionary<uint, InteractionHandEventType>();
        ...
        [DllImport(@"Kinect10.dll")]
        private static extern int NuiTransformSmooth(ref NuiSkeletonFrame pSkeletonFrame,
            ref NuiTransformSmoothParameters pSmoothingParams);

        [DllImport(@"Kinect10.dll")]
        private static extern int NuiImageGetColorPixelCoordinatesFromDepthPixelAtResolution(
            NuiImageResolution eColorResolution, NuiImageResolution eDepthResolution, ref NuiImageViewArea pcViewArea,
            int lDepthX, int lDepthY, ushort sDepthValue, out int plColorX, out int plColorY);


        [DllImport(@"KinectUnityWrapper.dll")]
        private static extern int InitKinectSensor(NuiInitializeFlags dwFlags, bool bEnableEvents, int iColorResolution,
            int iDepthResolution, bool bNearMode);
        ...
        [DllImport(@"KinectUnityWrapper.dll")]
        private static extern bool GetSkeletonFrameData(ref NuiSkeletonFrame pSkeletonData, ref uint iDataBufLen,
            bool bNewFrame);
        ...
        [DllImport(@"KinectUnityWrapper.dll")]
        private static extern bool GetInfraredFrameData(IntPtr shInfraredBuf, ref uint iInfraredBufLen,
            bool bGetNewFrame);
        ...
        public static extern bool GetBackgroundRemovalFrameDataNative(IntPtr btVideoBuf, ref uint iVideoBufLen,
            bool bGetNewFrame);
        ...
        public bool InitSensorInterface(bool bCopyLibs, ref bool bNeedRestart)
        {
            ...
            if (!bCopyLibs)
            {
                ...
                string sZipFileName = !KinectInterop.Is64bitArchitecture()
                    ? "KinectV1UnityWrapper.x86.zip"
                    : "KinectV1UnityWrapper.x64.zip";
...
//			return targetFile.Exists && targetFile.Length == iTargetSize;
            ...
            if (!KinectInterop.Is64bitArchitecture())
            {
                ...
                KinectInterop.UnzipResourceFiles(dictFilesToUnzip, "KinectV1FaceData.zip", ref bOneCopied,
                    ref bAllCopied);
                ...
                dictFilesToUnzip["KinectBackgroundRemoval180_32.dll"] =
                    sTargetPath + "KinectBackgroundRemoval180_32.dll";

                dictFilesToUnzip["msvcp100.dll"] = sTargetPath + "msvcp100.dll";
                dictFilesToUnzip["msvcr100.dll"] = sTargetPath + "msvcr100.dll";

                KinectInterop.UnzipResourceFiles(dictFilesToUnzip, "KinectV1UnityWrapper.x86.zip", ref bOneCopied,
                    ref bAllCopied);
            }
            else
            {
                ...
                dictFilesToUnzip["FaceTrackLib.dll"] = sTargetPath + "FaceTrackLib.dll";
                dictFilesToUnzip["KinectBackgroundRemoval180_64.dll"] =
                    sTargetPath + "KinectBackgroundRemoval180_64.dll";

                dictFilesToUnzip["msvcp100.dll"] = sTargetPath + "msvcp100.dll";
                dictFilesToUnzip["msvcr100.dll"] = sTargetPath + "msvcr100.dll";

                KinectInterop.UnzipResourceFiles(dictFilesToUnzip, "KinectV1UnityWrapper.x64.zip", ref bOneCopied,
                    ref bAllCopied);
            }
            ...
            bool bUseMultiSource)
        {
            ...
            FacetrackingManager[] faceManagers =
                UnityEngine.Object.FindObjectsByType<FacetrackingManager>(FindObjectsSortMode.None);
            ...
            SpeechManager[] speechManagers =
                UnityEngine.Object.FindObjectsByType<SpeechManager>(FindObjectsSortMode.None);
            ...
            int hr = InitKinectSensor(nuiFlags, true, (int) Constants.ColorImageResolution,
                (int) Constants.DepthImageResolution, Constants.IsNearMode);
            ...
            if (hr == 0)
            {
                ...
                NuiImageResolutionToSize(Constants.ColorImageResolution, out sensorData.colorImageWidth,
                    out sensorData.colorImageHeight);
...
//			sensorData.colorImageHeight = Constants.ColorImageHeight;
                ...
                NuiImageResolutionToSize(Constants.DepthImageResolution, out sensorData.depthImageWidth,
                    out sensorData.depthImageHeight);
...
//			sensorData.depthImageHeight = Constants.DepthImageHeight;
        ...
        private void GetHandStateAndConf(uint skeletonId, bool isRightHand, uint handState,
            InteractionHandEventType handEvent,
            ...
            ref Matrix4x4 kinectToWorld, bool bIgnoreJointZ)
        {
            ...
            if (newSkeleton)
            {
                ...
                for (int i = 0; i < sensorData.bodyCount; i++)
                {
                    ...
                    bodyFrame.bodyData[i].bIsTracked =
                        (short) (body.eTrackingState == NuiSkeletonTrackingState.SkeletonTracked ? 1 : 0);
                    ...
                    if (body.eTrackingState == NuiSkeletonTrackingState.SkeletonTracked)
                    {
                        ...
                        for (int j = 0; j < Constants.JointCount; j++)
                        {
                            ...
                            jointData.trackingState =
                                (KinectInterop.TrackingState) body.eSkeletonPositionTrackingState[j];
                            ...
                            if (jointData.trackingState != KinectInterop.TrackingState.NotTracked)
                            {
                                ...
                                float jPosZ = (bIgnoreJointZ && j > 0)
                                    ...
                                    : body.SkeletonPositions[j].z;
                                jointData.kinectPos = new Vector3(body.SkeletonPositions[j].x,
                                    body.SkeletonPositions[j].y, body.SkeletonPositions[j].z);
                                jointData.position = kinectToWorld.MultiplyPoint3x4(
                                    new Vector3(body.SkeletonPositions[j].x, body.SkeletonPositions[j].y, jPosZ));
                            }
...
//							                        (int)rightHandState, (int)rightHandEvent);
            ...
            ref Matrix4x4 kinectToWorld)
        {
                ...
                bodyFrame.bodyData[i].joint[l].trackingState != KinectInterop.TrackingState.NotTracked)
            {
                ...
                jointData.trackingState =
                    bodyFrame.bodyData[i].joint[r].trackingState == KinectInterop.TrackingState.Tracked &&
        ...
        private void NuiTransformSkeletonToDepthImage(Vector3 vPoint, out float pfDepthX, out float pfDepthY,
            out float pfDepthZ)
        {
        ...
        public void VisualizeFaceTrackerOnColorTex(Texture2D texColor)
        {
            if (bFaceTrackingInited)
            {
                ...
                if (GetFaceRect(GetFaceTrackingIDNative(), ref faceRect))
                {
                    ...
                    pt1.x = faceRect.x;
                    pt1.y = faceRect.y;
                    pt2.x = faceRect.x + faceRect.width - 1;
                    pt2.y = pt1.y;
                    ...
                    pt1.y = pt2.y;
                    pt2.x = pt1.x;
                    pt2.y = faceRect.y + faceRect.height - 1;
                    DrawLine(texColor, pt1, pt2, color);
        ...
        public bool GetAnimUnits(long userId, ref Dictionary<KinectInterop.FaceShapeAnimations, float> dictAU)
        {
            if (isUseFaceModel && dictAU != null && GetFaceTrackingIDNative() == userId)
            {
                ...
                if (iAUCount >= 6)
                {
                    ...
                    dictAU[KinectInterop.FaceShapeAnimations.LefteyebrowLowerer] =
                        afAU[3] - afAU[5]; // AU3 – Brow Lowerer
                    dictAU[KinectInterop.FaceShapeAnimations.RighteyebrowLowerer] =
                        afAU[3] - afAU[5]; // AU3 – Brow Lowerer

                    dictAU[KinectInterop.FaceShapeAnimations.LipCornerDepressorLeft] =
                        afAU[4]; // AU4 – Lip Corner Depressor
                    dictAU[KinectInterop.FaceShapeAnimations.LipCornerDepressorRight] =
                        afAU[4]; // AU4 – Lip Corner Depressor
