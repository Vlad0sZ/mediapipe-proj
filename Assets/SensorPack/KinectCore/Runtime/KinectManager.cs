...
using System.Linq;
using SensorPack.KinectCore.Runtime.Avatar;
using SensorPack.KinectCore.Runtime.Behaviours;
using SensorPack.KinectCore.Runtime.Filters;
using SensorPack.KinectCore.Runtime.Interfaces;
using SensorPack.KinectCore.Runtime.Managers;
using UnityEngine;

namespace SensorPack.KinectCore.Runtime
{
    /// <summary>
    ...
    {
        public enum AutoHeightAngle : int
        {
            DontUse,
            ShowInfoOnly,
            AutoUpdate,
        ...
        public enum UserDetectionOrder : int
        {
            Appearance = 0,
            Distance = 1,
            LeftToRight = 2,
        ...
        public enum AllowedRotations : int
        {
            None = 0,
            Default = 1,
            All = 2
        ...
        [Tooltip("List of objects that can be provided by available sensors")] [SerializeField]
        private List<SensorInterfacesHolder> sensorInterfacesList;
        ...
        [Tooltip("Whether to automatically set the sensor height and angle or not. " +
                 "The user must stay in front of the sensor, in order the automatic detection to work.")]
        ...
        [Tooltip("Depth and color image width on the screen, as percent of the screen width. " +
                 "The image height is calculated depending on the width.")]
        ...
        [Tooltip("Maximum distance to a user, in order to be considered for skeleton data processing." +
                 " Value of 0 means no maximum distance limitation.")]
        public float maxUserDistance = 0f;

        [Tooltip("Maximum left or right distance to a user, in order to be considered for skeleton data processing." +
                 " Value of 0 means no left/right distance limitation.")]
        ...
        [Tooltip("Whether to update the AvatarControllers in LateUpdate(), instead of in Update(). " +
                 "Needed for Mecanim animation blending.")]
        ...
        [Tooltip("Allowed wrist and hand rotations: None - no hand rotations are allowed, " +
                 "Default - hand rotations are allowed except for the twists, " +
                 "All - all rotations are allowed.")]
        public AllowedRotations allowedHandRotations = AllowedRotations.Default;

        [Tooltip("List of the avatar controllers in the scene. " +
                 "If the list is empty, the available avatar controllers are detected at the scene start up.")]
        ...
        private RenderTexture usersLblTex = null;
        private Texture2D usersLblTex2D = null;
        ...
        private Dictionary<Int64, KinectGestures.GestureData> playerCalibrationData =
            new Dictionary<Int64, KinectGestures.GestureData>();

        // gestures data and parameters
        private Dictionary<Int64, List<KinectGestures.GestureData>> playerGesturesData =
            new Dictionary<Int64, List<KinectGestures.GestureData>>();
        ...
        private JointVelocitiesFilter jointVelocityFilter = null;
        ...
        private bool backgroundRemovalHiRes = false;

        public static bool SensorAvailable
        {
            get
            {
                if (Instance == null || Instance.kinectInitialized == false) 
                    return false;

                var firstSensor = Instance.sensorInterfaces?.FirstOrDefault();
                return Instance.sensorData != null && (firstSensor?.DeviceIsActive() ?? false);
            }
        }
        ...
        /// <summary>
        /// Gets the sensor data structure (this structure should not be modified and must be used only privately).
        /// </summary>
...
//	}
        ...
        public Texture2D GetUsersLblTex2D()
        {
            ...
            {
                usersLblTex2D = new Texture2D(sensorData.depthImageWidth, sensorData.depthImageHeight, TextureFormat.ARGB32,
                    false);
                bNewTexture = true;
            }
        ...
        public Texture2D GetUsersClrTex2D()
        {
            ...
            sensorData.colorImageTexture2D = new Texture2D(sensorData.colorImageWidth, sensorData.colorImageHeight,
...
//		}
        ...
        /// <summary>
        /// Gets the user body data (for private purposes only).
        /// </summary>
        ...
        public KinectInterop.BodyData GetUserBodyDataByIndex(int index)
        {
            if (index < 0 || index >= bodyFrame.bodyData.Length)
                return default;
            ...
            if (dictUserIdToIndex.ContainsKey(userId))
            {
                    ...
                    bodyFrame.bodyData[index].bIsTracked != 0)
                {
                    KinectInterop.JointData jointData =
                        bodyFrame.bodyData[index].joint[(int)KinectInterop.JointType.SpineBase];
        ...
        public bool GetLeftHandInteractionBox(Int64 userId, ref Vector3 leftBotBack, ref Vector3 rightTopFront,
            bool bValidBox)
        {
            if (dictUserIdToIndex.ContainsKey(userId))
            {
                    ...
                    bodyFrame.bodyData[index].bIsTracked != 0)
                {
                    ...
                    if (bodyData.joint[(int)KinectInterop.JointType.ShoulderRight].trackingState !=
                        KinectInterop.TrackingState.NotTracked &&
                        bodyData.joint[(int)KinectInterop.JointType.HipLeft].trackingState !=
                        KinectInterop.TrackingState.NotTracked)
                    {
                        rightTopFront.x = bodyData.joint[(int)KinectInterop.JointType.ShoulderRight].position.x;
                        leftBotBack.x = rightTopFront.x -
                                        2 * (rightTopFront.x -
                                             bodyData.joint[(int)KinectInterop.JointType.HipLeft].position.x);
                    }
                    else
                    {
                        bResult = bValidBox;
                    ...
                    if (bodyData.joint[(int)KinectInterop.JointType.HipRight].trackingState !=
                        KinectInterop.TrackingState.NotTracked &&
                        bodyData.joint[(int)KinectInterop.JointType.ShoulderRight].trackingState !=
                        KinectInterop.TrackingState.NotTracked)
                    {
                    ...
                    if (bodyData.joint[(int)KinectInterop.JointType.SpineBase].trackingState !=
                        KinectInterop.TrackingState.NotTracked)
                    {
                        //leftBotBack.z = bodyData.joint[(int)KinectInterop.JointType.SpineBase].position.z;
                        leftBotBack.z = !ignoreZCoordinates
                            ? bodyData.joint[(int)KinectInterop.JointType.SpineBase].position.z
        ...
        /// <param name="bValidBox">If set to <c>true</c>, the previously set coordinates are valid</param>
        public bool GetRightHandInteractionBox(Int64 userId, ref Vector3 leftBotBack, ref Vector3 rightTopFront,
            bool bValidBox)
        {
            if (dictUserIdToIndex.ContainsKey(userId))
            {
                    ...
                    bodyFrame.bodyData[index].bIsTracked != 0)
                {
                    ...
                    if (bodyData.joint[(int)KinectInterop.JointType.ShoulderLeft].trackingState !=
                        KinectInterop.TrackingState.NotTracked &&
                        bodyData.joint[(int)KinectInterop.JointType.HipRight].trackingState !=
                        KinectInterop.TrackingState.NotTracked)
                    {
                        leftBotBack.x = bodyData.joint[(int)KinectInterop.JointType.ShoulderLeft].position.x;
                        rightTopFront.x = leftBotBack.x +
                                          2 * (bodyData.joint[(int)KinectInterop.JointType.HipRight].position.x -
                                               leftBotBack.x);
                    }
                    else
                    {
                    ...
                    if (bodyData.joint[(int)KinectInterop.JointType.HipLeft].trackingState !=
                        KinectInterop.TrackingState.NotTracked &&
                        bodyData.joint[(int)KinectInterop.JointType.ShoulderLeft].trackingState !=
                        KinectInterop.TrackingState.NotTracked)
                    {
        ...
        {
            List<KinectGestures.GestureData> gesturesData = playerGesturesData.ContainsKey(UserId)
                ? playerGesturesData[UserId]
                : new List<KinectGestures.GestureData>();
        ...
        {
            List<KinectGestures.GestureData> gesturesData =
                playerGesturesData.ContainsKey(UserId) ? playerGesturesData[UserId] : null;
            ...
            gestureData.startTrackingAtTime =
                Time.realtimeSinceStartup + KinectInterop.Constants.MinTimeBetweenSameGestures;
        ...
        {
            return KinectInterop.GetBodyFrameAsK4b(sensorData, ref bodyFrame, ref liRelTime, Vector3.one, ref fUnityTime,
                delimiter);
        }
        ...
        private void Awake()
        {
            ...
            try
            {
                ...
                var interfaces = GetSensorInterfaces();
                sensorInterfaces = KinectInterop.InitSensorInterfaces(interfaces, bOnceRestarted, ref bNeedRestart);
        ...
        public void StartKinect()
        {
            try
            {
                ...
                sensorData = KinectInterop.OpenDefaultSensor(sensorInterfaces, dwFlags, sensorAngle, useMultiSourceReader,
                    computeUserMap, brManager);

                if (sensorData == null)
                {
                    if (sensorInterfaces == null || sensorInterfaces.Count == 0)
                        throw new Exception(
                            "No sensor found. Make sure you have installed the SDK and the sensor is connected.");
                    else
            ...
            bodyFrame.bJointVelocities = estimateJointVelocities /**|| allowTurnArounds*/
                ; // bJointVelocities is needed to turn on velocities computing ( setup jointData.posDrv in KinectInterop.cs ) -> done this way to not modify other files 
            ...
            {
                MonoBehaviour[] monoScripts = UnityEngine.Object.FindObjectsByType<MonoBehaviour>(FindObjectsSortMode.None);
            ...
            {
                Debug.Log("Found " + gestureListeners.Count +
                          " gesture listener(s), but no gesture manager in the scene. Adding KinectGestures-component...");
        ...
        private DepthSensorInterface[] GetSensorInterfaces()
        {
            if (sensorInterfacesList == null || sensorInterfacesList.Count == 0) 
                sensorInterfacesList = GetComponentsInChildren<SensorInterfacesHolder>().ToList();


            foreach (SensorInterfacesHolder holder in sensorInterfacesList)
            {
                var interfaces = holder.GetAvailableInterfaces();
                if (interfaces != null && interfaces.Length > 0)
                    return interfaces;
            }

            return KinectInterop.SensorInterfaceOrder;
            ...
            if (kinectInitialized)
            {
                    ...
                    (computeUserMap != UserMapType.None && computeUserMap != UserMapType.RawUserDepth))
                {
                    if (usersMapRect.width == 0 || usersMapRect.height == 0)
                    {
                        ...
                        if (this.displayMapsWidthPercent == 0f)
                        {
                            this.displayMapsWidthPercent = (sensorData.depthImageWidth / 2f) * 75 / cameraRect.width;
                        }

                        float displayMapsWidthPercent = this.displayMapsWidthPercent / 100f;
                        float displayMapsHeightPercent = displayMapsWidthPercent * sensorData.depthImageHeight /
                                                         sensorData.depthImageWidth;

                        float displayWidth = cameraRect.width * displayMapsWidthPercent;
                        float displayHeight = cameraRect.width * displayMapsHeightPercent;

                        usersMapRect = new Rect(0, cameraRect.height, displayWidth, -displayHeight);
                    }
                ...
                else if (computeColorMap && displayColorMap)
                {
                    if (usersClrRect.width == 0 || usersClrRect.height == 0)
                    {
                        ...
                        if (this.displayMapsWidthPercent == 0f)
                        {
                            this.displayMapsWidthPercent = (sensorData.depthImageWidth / 2) * 100 / cameraRect.width;
                        }

                        float displayMapsWidthPercent = this.displayMapsWidthPercent / 100f;
                        float displayMapsHeightPercent = displayMapsWidthPercent * sensorData.colorImageHeight /
                                                         sensorData.colorImageWidth;
...
//					}
                    ...
                    if (sensorData.colorImageTexture2D != null || sensorData.colorImageTexture != null)
                    {
                        ...
                        GUI.DrawTexture(usersClrRect,
                            sensorData.colorImageTexture2D ? sensorData.colorImageTexture2D : sensorData.colorImageTexture);
                    }
                }
            }
        }
        ...
        private void UpdateKinectStreams()
        {
            ...
            bLimitedUsers = showTrackedUsersOnly &&
                            ((maxTrackedUsers < 6 /**&& (alUserIds.Count > maxTrackedUsers)*/) ||
            ...
            // poll color map
            if (computeColorMap)
                sensorData.newColorImage = KinectInterop.PollColorFrame(sensorData);

            // poll infrared map
            if (computeInfraredMap) 
                sensorData.newInfraredImage = KinectInterop.PollInfraredFrame(sensorData);
        
            // poll user map
            if (computeUserMap != UserMapType.None)
            {
                sensorData.firstUserIndex = liPrimaryUserId != 0 && dictUserIdToIndex.TryGetValue(liPrimaryUserId, out int value)
                    ? value
                    : -1;

                sensorData.newDepthImage = KinectInterop.PollDepthFrame(sensorData, computeUserMap, bLimitedUsers,
                    dictUserIdToIndex.Values);
            }
            ...
            if (sensorData == null || !sensorData.isPlayModeEnabled)
            {
                sensorData.newBodyFrame =
                    KinectInterop.PollBodyFrame(sensorData, ref bodyFrame, ref kinectToWorld, ignoreZCoordinates);
            }
            else
            {
                if (!string.IsNullOrEmpty(sensorData.playModeData))
                {
                    if (sensorData.playModeData.StartsWith("k4b"))
                        sensorData.newBodyFrame = KinectInterop.SetBodyFrameFromK4b(sensorData.playModeData, ";",
                            sensorData, ref bodyFrame, ref kinectToWorld, ignoreZCoordinates);
                    else
                        sensorData.newBodyFrame = KinectInterop.SetBodyFrameFromCsv(sensorData.playModeData, sensorData,
                            ref bodyFrame, ref kinectToWorld);
...
//						}
                        ...
                        {
                            if (listener != null && listener.GestureCompleted(userId, userIndex, gestureData.gesture,
                    ...
                    else if (gestureData.cancelled)
                    {
                        foreach (KinectGestures.GestureListenerInterface listener in gestureListeners)
                        {
                            if (listener != null && listener.GestureCancelled(userId, userIndex, gestureData.gesture,
                                    (KinectInterop.JointType)gestureData.joint))
                            {
...
//						}
        ...
        void UpdateUserHistogramImage(UserMapType userMapType)
        {
            ...
            for (int i = 0; i < usersMapSize; i++)
            {
                ...
                if (nowUserPixel != wasUserPixel)
                {
                    ...
                    bool bUserTracked = btSelBI != 255
                        ? btSelBI == (byte)userMap
                        :
                        ...
                        (bLimitedUsers && (userMap < sensorData.bodyCount)
                            ? sensorData.trackedBodyIndices[userMap] != 0
                            : userMap != 255);
                    ...
                    else
                    {
                        if (userMapType == UserMapType.CutOutTexture && sensorData.colorImage != null)
                        {
                            ...
                            if (!float.IsInfinity(vColorPos.x) && !float.IsInfinity(vColorPos.y))
                            {
                                ...
                                {
                                    int ci = colorIndex << 2;
                                    Color32 colorPixel = new Color32(sensorData.colorImage[ci],
                                        sensorData.colorImage[ci + 1], sensorData.colorImage[ci + 2], 255);
        ...
        private List<long> _addedUsers;
        private List<int> _addedIndexes;
        private List<long> _lostUsers;

        // Processes body frame data
        private void ProcessBodyFrameData()
        {
            _addedUsers ??= new List<long>(12);
            _addedIndexes ??= new List<int>(12);
            _lostUsers ??= new List<long>(12);
        
            _addedUsers.Clear();
            _addedIndexes.Clear();
            _lostUsers.Clear();
            _lostUsers.AddRange(alUserIds);
        
            if ((autoHeightAngle == AutoHeightAngle.ShowInfoOnly ||
                 autoHeightAngle == AutoHeightAngle.AutoUpdateAndShowInfo) &&
                ...
                calibrationText != null)
            {
                ...
                calibrationText.text = string.Format("Sensor Height: {0:F2} m, Angle: {1:F0} deg",
                    sensorData.sensorHgtDetected, -angle);
            }

            if ((autoHeightAngle == AutoHeightAngle.AutoUpdate ||
                 autoHeightAngle == AutoHeightAngle.AutoUpdateAndShowInfo) &&
                (sensorData.sensorHgtDetected != 0f || sensorData.sensorRotDetected.eulerAngles.x != 0f))
            {
                float angle = sensorData.sensorRotDetected.eulerAngles.x;
...
//				}
                    ...
                    if (!_addedUsers.Contains(userId))
                    {
...
////// 		turnaround mode start
                    ...
                    if (allowTurnArounds || usersFaceBackwards)
                    {
                        if (!usersFaceBackwards)
                        {
                            ...
                            bool bLeftShoulderTracked =
                                (bodyData.joint[(int)KinectInterop.JointType.ShoulderLeft].trackingState ==
                                 KinectInterop.TrackingState.Tracked);
                            if (bLeftShoulderTracked)
                            {
                                bodyData.turnLeftShoulderTrackedTime = Time.realtimeSinceStartup;
                            }

                            bool bRightShoulderTracked =
                                (bodyData.joint[(int)KinectInterop.JointType.ShoulderRight].trackingState ==
                                 KinectInterop.TrackingState.Tracked);
                            ...
                            float timeSinceTrackedLShoulder =
                                Time.realtimeSinceStartup - bodyData.turnLeftShoulderTrackedTime;
                            float timeSinceTrackedRShoulder =
                                Time.realtimeSinceStartup - bodyData.turnRightShoulderTrackedTime;
                            float timeSinceTrackedShoulderDist =
                                Time.realtimeSinceStartup - bodyData.turnShoulderDistTrackedTime;

                            // allow up to 0.5 second deviation
                            bodyData.isTurnedAround = bFaceTrackingActive && (timeSinceTrackedFace > 0.5f) &&
                                                      (timeSinceTrackedLShoulder < 0.5f) &&
                                                      (timeSinceTrackedRShoulder < 0.5f);
                        ...
                        if (bodyData.isTurnedAround)
                        {
                            ...
                            SwitchJointsData(ref bodyData, (int)KinectInterop.JointType.ShoulderLeft,
                                (int)KinectInterop.JointType.ShoulderRight);
                            SwitchJointsData(ref bodyData, (int)KinectInterop.JointType.ElbowLeft,
                                (int)KinectInterop.JointType.ElbowRight);
                            SwitchJointsData(ref bodyData, (int)KinectInterop.JointType.WristLeft,
                                (int)KinectInterop.JointType.WristRight);
                            SwitchJointsData(ref bodyData, (int)KinectInterop.JointType.HandLeft,
                                (int)KinectInterop.JointType.HandRight);
                            SwitchJointsData(ref bodyData, (int)KinectInterop.JointType.ThumbLeft,
                                (int)KinectInterop.JointType.ThumbRight);
                            SwitchJointsData(ref bodyData, (int)KinectInterop.JointType.HandTipLeft,
                                (int)KinectInterop.JointType.HandTipRight);

                            SwitchJointsData(ref bodyData, (int)KinectInterop.JointType.HipLeft,
                                (int)KinectInterop.JointType.HipRight);
                            SwitchJointsData(ref bodyData, (int)KinectInterop.JointType.KneeLeft,
                                (int)KinectInterop.JointType.KneeRight);
                            SwitchJointsData(ref bodyData, (int)KinectInterop.JointType.AnkleLeft,
                                (int)KinectInterop.JointType.AnkleRight);
                            SwitchJointsData(ref bodyData, (int)KinectInterop.JointType.FootLeft,
...
////// 		turnaround mode end
            ...
            if (_lostUsers.Count > 0)
            {
                foreach (Int64 userId in _lostUsers)
                {
            ...
            {
                for (int i = 0; i < _addedUsers.Count; i++)
                {
                    if (alUserIds.Count < maxTrackedUsers)
                    {
                        Int64 userId = _addedUsers[i];
                        int userIndex = _addedIndexes[i];
                ...
                for (int i = 0; i < _addedUsers.Count; i++)
                {
                    ...
                    if (ui >= 0 && ui != userIndex)
                    {
                        ...
                        Debug.Log("Updating user " + uidIndex + ", ID: " + userId + ", Body: " + userIndex + ", Time: " +
                                  Time.time);
            ...
            foreach (var point in bodyFrame.bodyData)
            {
                //Debug.LogError($"New joint pos at: {(Vector2)point.position}");
            }
        ...
        private void ProcessBodySpecialData(ref KinectInterop.BodyData bodyData)
        {
            ...
            if ((bodyData.joint[(int)KinectInterop.JointType.ShoulderLeft].trackingState !=
                 KinectInterop.TrackingState.Tracked &&
                 bodyData.joint[(int)KinectInterop.JointType.SpineShoulder].trackingState ==
                 KinectInterop.TrackingState.Tracked &&
                 bodyData.joint[(int)KinectInterop.JointType.ShoulderRight].trackingState ==
                 KinectInterop.TrackingState.Tracked &&
                 bodyData.joint[(int)KinectInterop.JointType.SpineMid].trackingState ==
                 KinectInterop.TrackingState.Tracked))
            {
                bodyData.joint[(int)KinectInterop.JointType.ShoulderLeft].trackingState =
                    KinectInterop.TrackingState.Inferred;
                ...
                bodyData.joint[(int)KinectInterop.JointType.ShoulderLeft].position =
                    kinectToWorld.MultiplyPoint3x4(posShoulderL);
                bodyData.joint[(int)KinectInterop.JointType.ShoulderLeft].direction =
                    bodyData.joint[(int)KinectInterop.JointType.ShoulderLeft].position -
                 ...
                 KinectInterop.TrackingState.Tracked))
            {
                ...
                bodyData.joint[(int)KinectInterop.JointType.ShoulderRight].position =
                    kinectToWorld.MultiplyPoint3x4(posShoulderR);
                bodyData.joint[(int)KinectInterop.JointType.ShoulderRight].direction =
                    bodyData.joint[(int)KinectInterop.JointType.ShoulderRight].position -
                    bodyData.joint[(int)KinectInterop.JointType.SpineShoulder].position;
            }

            if (bodyData.joint[(int)KinectInterop.JointType.HipLeft].trackingState != KinectInterop.TrackingState.Tracked &&
                bodyData.joint[(int)KinectInterop.JointType.SpineBase].trackingState ==
                KinectInterop.TrackingState.Tracked &&
                bodyData.joint[(int)KinectInterop.JointType.HipRight].trackingState ==
                KinectInterop.TrackingState.Tracked &&
                bodyData.joint[(int)KinectInterop.JointType.SpineMid].trackingState == KinectInterop.TrackingState.Tracked)
            {
                ...
                bodyData.joint[(int)KinectInterop.JointType.HipLeft].direction =
                    bodyData.joint[(int)KinectInterop.JointType.HipLeft].position -
                ...
                bodyData.joint[(int)KinectInterop.JointType.SpineMid].trackingState == KinectInterop.TrackingState.Tracked)
            {
                ...
                bodyData.joint[(int)KinectInterop.JointType.HipRight].direction =
                    bodyData.joint[(int)KinectInterop.JointType.HipRight].position -
                    bodyData.joint[(int)KinectInterop.JointType.SpineBase].position;
            }
...
//				}
            ...
            if (bodyData.joint[(int)KinectInterop.JointType.KneeLeft].trackingState !=
                KinectInterop.TrackingState.NotTracked &&
                bodyData.joint[(int)KinectInterop.JointType.AnkleLeft].trackingState !=
                KinectInterop.TrackingState.NotTracked &&
                bodyData.joint[(int)KinectInterop.JointType.FootLeft].trackingState !=
                KinectInterop.TrackingState.NotTracked)
            {
                Vector3 vFootProjected = Vector3.Project(bodyData.joint[(int)KinectInterop.JointType.FootLeft].direction,
                    bodyData.joint[(int)KinectInterop.JointType.AnkleLeft].direction);
            ...
            if (bodyData.joint[(int)KinectInterop.JointType.KneeRight].trackingState !=
                KinectInterop.TrackingState.NotTracked &&
                bodyData.joint[(int)KinectInterop.JointType.AnkleRight].trackingState !=
                KinectInterop.TrackingState.NotTracked &&
                bodyData.joint[(int)KinectInterop.JointType.FootRight].trackingState !=
                KinectInterop.TrackingState.NotTracked)
            {
                Vector3 vFootProjected = Vector3.Project(bodyData.joint[(int)KinectInterop.JointType.FootRight].direction,
                    bodyData.joint[(int)KinectInterop.JointType.AnkleRight].direction);
                ...
                for (int i = aUserIndexIds.Length - 1; i >= 1; i--)
                {
                    ...
                    for (int j = 0; j < i; j++)
                    {
                        ...
                        if (userDetectionOrder == UserDetectionOrder.Distance)
                            userDist1 = Mathf.Abs(userPos[j].x - centralPosition.x) +
                                        Mathf.Abs(userPos[j].z - centralPosition.z);
                        ...
                        float userDist2 = 0f;
                        if (userDetectionOrder == UserDetectionOrder.Distance)
                            userDist2 = Mathf.Abs(userPos[j + 1].x - centralPosition.x) +
                                        Mathf.Abs(userPos[j + 1].z - centralPosition.z);
                        else if (userDetectionOrder == UserDetectionOrder.LeftToRight)
        ...
        private int GetEmptyUserSlot(Int64 userId, int bodyIndex)
        {
            ...
            else if (userDetectionOrder != UserDetectionOrder.Appearance)
            {
                ...
                for (int i = 0; i < aUserIndexIds.Length; i++)
                {
                    ...
                    else
                    {
                        ...
                        float uidUserDist = 0;
                        if (userDetectionOrder == UserDetectionOrder.Distance)
                            uidUserDist = Mathf.Abs(uidUserPos.x - centralPosition.x) +
                                          Mathf.Abs(uidUserPos.z - centralPosition.z);
                        else if (userDetectionOrder == UserDetectionOrder.LeftToRight)
                            ...
                            for (int u = (aUserIndexIds.Length - 2); u >= i; u--)
                            {
                                ...
                                if (aUserIndexIds[u] != 0)
                                {
                                    Debug.Log(string.Format(
                                        "Reindexing user {0} to {1}, ID: {2}, pos: {3}, other-pos: {4}.", u, u + 1,
                                        aUserIndexIds[u], userPos, uidUserPos));
                                }
                            }
        ...
        private  void FreeEmptyUserSlot(int uidIndex)
        {
        ...
        private void CalibrateUser(Int64 userId, int bodyIndex)
        {
            if (!alUserIds.Contains(userId))
            {
                if (CheckForCalibrationPose(userId, bodyIndex, playerCalibrationPose))
                {
                    ...
                    Debug.Log("Adding user " + uidIndex + ", ID: " + userId + ", Body: " + bodyIndex + ", Time: " +
...
//				}
        ...
        private void DrawSkeleton(Texture2D aTexture, ref KinectInterop.BodyData bodyData)
        {
            ...
            for (int i = 0; i < jointsCount; i++)
            {
                ...
                bool isJointTracked = ignoreInferredJoints
                ...
                bool isParentTracked = ignoreInferredJoints
                    ? (bodyData.joint[parent].trackingState == KinectInterop.TrackingState.Tracked)
                    : (bodyData.joint[parent].trackingState != KinectInterop.TrackingState.NotTracked);

                if (isJointTracked && isParentTracked)
                {
                    Vector2 posParent =
                        KinectInterop.MapSpacePointToDepthCoords(sensorData, bodyData.joint[parent].kinectPos);
                    ...
                    if (posParent != Vector2.zero && posJoint != Vector2.zero)
                    {
                        ...
                        KinectInterop.DrawLine(aTexture, (int)posParent.x, (int)posParent.y, (int)posJoint.x,
                            (int)posJoint.y, Color.yellow);
                    }
                }
            }
        ...
        private void CalculateJointOrients(ref KinectInterop.BodyData bodyData)
        {
            ...
            for (int j = 0; j < jointCount; j++)
            {
                ...
                bool bJointValid = ignoreInferredJoints
                    ? jointData.trackingState == KinectInterop.TrackingState.Tracked
                    : jointData.trackingState != KinectInterop.TrackingState.NotTracked;
                ...
                if (bJointValid)
                {
                    ...
                    if (nextJoint != joint && nextJoint >= 0 && nextJoint < sensorData.jointCount)
                    {
                        ...
                        bool bNextJointValid = ignoreInferredJoints
                            ? nextJointData.trackingState == KinectInterop.TrackingState.Tracked
                            : nextJointData.trackingState != KinectInterop.TrackingState.NotTracked;
...
//								bodyData.joint[(int)KinectInterop.JointType.SpineShoulder].trackingState == KinectInterop.TrackingState.NotTracked) 
                                if ((parDotJoint >= 0.01f && parDotJoint <= 0.99f) ||
                                    (parDotJoint >= -0.99f && parDotJoint <= -0.01f))
                                {
...
//									Debug.DrawRay (jointData.position * 3f, fwdDir * 0.5f, Color.blue);
                                    ...
                                    {
                                        KinectInterop.JointData shCenterData =
                                            bodyData.joint[(int)KinectInterop.JointType.SpineShoulder];
...
//									Debug.DrawRay (jointData.position * 3f, fwdDir * 0.5f, Color.blue);
                            ...
                            bool bRotated = allowedHandRotations == AllowedRotations.None &&
                                            joint != (int)KinectInterop.JointType.ShoulderLeft &&
                                            joint != (int)KinectInterop.JointType.ElbowLeft;
                            if (bRotated)
                            {
                                    ...
                                    prevJointData.trackingState != KinectInterop.TrackingState.NotTracked)
                                {
                                    ...
                                    if (rightDir != Vector3.zero && fwdDir != Vector3.zero)
                                    {
                                        ...
                                        jointOrientNormal = (joint == (int)KinectInterop.JointType.WristLeft)
                                            ? Quaternion.RotateTowards(prevJointData.normalRotation, jointOrientThumb, 80f)
                                            : jointOrientThumb;
                                ...
                                joint != (int)KinectInterop.JointType.ElbowLeft)
                            {
                                ...
                                {
                                    jointData.normalRotation = Quaternion.RotateTowards(prevJointData.normalRotation,
                                        jointData.normalRotation, 70f);
                                }
                            }
                        }
...
//									Debug.DrawRay (jointData.position * 3f, fwdDir * 0.5f, Color.blue);
                            ...
                            if (joint != (int)KinectInterop.JointType.ShoulderRight &&
                                joint != (int)KinectInterop.JointType.ElbowRight)
                            {
                                // limit wrist and hand twist
                                int prevJoint = (int)KinectInterop.GetParentJoint((KinectInterop.JointType)joint);
                            ...
                            (joint == (int)KinectInterop.JointType.Neck))
                        {
                            ...
                            Vector3
                                jointDir2 = bodyData
                                    .shouldersDirection; // Vector3.Lerp(bodyData.shouldersDirection, -bodyData.shouldersDirection, bodyData.turnAroundFactor);
                            jointDir2.z = -jointDir2.z;
...
//						jointData.normalRotation = jointOrientNormal;
                        ...
                        else if ((joint == (int)KinectInterop.JointType.HipLeft) ||
                                 (joint == (int)KinectInterop.JointType.HipRight) ||
                                 (joint == (int)KinectInterop.JointType.KneeLeft) ||
                                 (joint == (int)KinectInterop.JointType.KneeRight) ||
                                 (joint == (int)KinectInterop.JointType.AnkleLeft) ||
                                 (joint == (int)KinectInterop.JointType.AnkleRight))
                        {
                            Vector3 baseDir2 = Vector3.right;
                            Vector3
                                jointDir2 = bodyData
                                    .shouldersDirection; // Vector3.Lerp(bodyData.hipsDirection, -bodyData.hipsDirection, bodyData.turnAroundFactor);
                            jointDir2.z = -jointDir2.z;
...
//						jointData.normalRotation = jointOrientNormal;
                            ...
                            sensorData != null && sensorData.sensorInterface != null)
                        {
                                    ...
                                    if (sensorData.sensorInterface.GetHeadRotation(bodyData.liTrackingID, ref headRotation))
                                    {
                                        if (headRotation != Quaternion.identity)
                                        {
                                            ...
                                            bodyData.headOrientation = bodyData.headOrientation != Quaternion.identity
                                                ? Quaternion.Slerp(bodyData.headOrientation, Quaternion.Euler(rotAngles),
                                                    10f * Time.deltaTime)
                            ...
                            {
                                Quaternion turnBodyRot = !bodyData.isTurnedAround
                                    ? Quaternion.identity
...
//						else
                ...
                for (int i = 0; i < iNeededJointsCount; i++)
                {
                    ...
                    if (joint >= 0)
                    {
                        ...
                        if (!playerJointsTracked[joint] && (joint == (int)KinectInterop.JointType.SpineShoulder) &&
                            IsJointTracked(UserId, (int)KinectInterop.JointType.ShoulderLeft) &&
                            IsJointTracked(UserId, (int)KinectInterop.JointType.ShoulderRight))
                        {
                            playerJointsTracked[joint] = true;
                            playerJointsPos[joint] = (GetJointPosition(UserId, (int)KinectInterop.JointType.ShoulderLeft) +
                                                      GetJointPosition(UserId,
                                                          (int)KinectInterop.JointType.ShoulderRight)) / 2f;
                        }
                    }
                }
        ...
        private bool IsConflictingGestureInProgress(KinectGestures.GestureData gestureData,
            ref List<KinectGestures.GestureData> gesturesData)
        {
        ...
        private bool CheckForCalibrationPose(Int64 UserId, int bodyIndex,
            KinectGestures.Gestures calibrationGesture)
        {
            ...
            KinectGestures.GestureData gestureData = playerCalibrationData.ContainsKey(UserId)
                ? playerCalibrationData[UserId]
                : new KinectGestures.GestureData();
            ...
            for (int i = 0; i < iNeededJointsCount; i++)
            {
                ...
                if (joint >= 0)
                {
                    ...
                    if (!playerJointsTracked[joint] && (joint == (int)KinectInterop.JointType.SpineShoulder))
                    {
                        KinectInterop.JointData lShoulderData =
                            bodyFrame.bodyData[bodyIndex].joint[(int)KinectInterop.JointType.ShoulderLeft];
                        KinectInterop.JointData rShoulderData =
                            bodyFrame.bodyData[bodyIndex].joint[(int)KinectInterop.JointType.ShoulderRight];

                        if (lShoulderData.trackingState != KinectInterop.TrackingState.NotTracked &&
                            rShoulderData.trackingState != KinectInterop.TrackingState.NotTracked)
                        {
                            playerJointsTracked[joint] = true;
...
//#endif
        ...
        public static event Action<SensorStatus> SensorStatusWasChanged;
        private SensorStatus? _lastStatus = null;
        ...
        private IEnumerator Check()
        {
            ...
            while (true)
            {
                if (iter >= frameDelay)
                {
                    if (sensorData != null)
                    {
                        var result = sensorData.sensorInterface.DeviceIsActive();
                        ChangeSensorState(result ? SensorStatus.Ready : SensorStatus.NotConnected);
                    }
                    else
                    {
                        if (sensorInterfaces == null || sensorInterfaces.Count == 0)
                            ChangeSensorState(SensorStatus.NotConnected);
                        else
                            ChangeSensorState(SensorStatus.Busy);
                    }
        ...
        private void ChangeSensorState(SensorStatus status)
        {
            if (_lastStatus.HasValue && _lastStatus.Value == status) return;
            _lastStatus = status;
            SensorStatusWasChanged?.Invoke(status);
        }

        public KinectInterop.DepthSensorPlatform GetKinectType()
        {
            return sensorData?.sensorIntPlatform ?? KinectInterop.DepthSensorPlatform.None;
        }
    }
}
